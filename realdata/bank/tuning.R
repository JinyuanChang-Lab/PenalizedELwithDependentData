...
rep.tuning <- function(index, para) 
{
  ...
  esti <- try(main_iter(auxi.fun, XX, init.theta, grad_g, tau, nu, eps.tol, iter_num), silent = T)
  if(class(esti) == "try-error") obj.val <- Inf
  else{
...
tun.para <- function(tau.vec = c(0.1/sqrt(n)*log(r), 1.0/sqrt(n)*log(r)), nu.vec = c(0.1/sqrt(n)*log(r), 1.0/sqrt(n)*log(r)), 
                     ntau = 10, nnu = 10, core.num = NULL, criterion = "BIC",
                     auxi.fun, XX, init.theta, grad_g, eps.tol = 0.005, iter_num = 600) #tuning para
{
  ...
  tau.vect <- seq(tau.min, tau.max, len = ntau)
  nu.vect <- seq(nu.min, nu.max, len = nnu)
