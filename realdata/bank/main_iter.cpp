...
Rcpp::List grad_theta(mat theta, uvec supp_theta, mat lambda, mat g_ee, cube gradG, double tau){
  ...
  uvec proxset;
  if(count == 0) proxset = {};
  else proxset = prox_set.head(count);
...
Rcpp::List main_iter(Rcpp::Function auxi_fun, mat XX, mat init_theta, Rcpp::Function grad_g,
  ...
  cube gradG;
  double beta1 = 0.9, beta2 = 0.999, epsilon = 1e-6, alpha = 0.0005;
  vec m(p, fill::zeros);
  vec v(p, fill::zeros);
  vec m_hat, v_hat;
  ...
  for(int count_num = 0; count_num < iter_num; count_num++){
    ...
    if(prox_set.n_elem > 0) theta.rows(prox_set).fill(0);
    supp_theta = find(vectorise(theta) != 0);
    
    if(supp_theta.n_elem == 0) {break;}
    ...
    if(zero_theta.n_elem > 0) theta.rows(zero_theta).fill(0);
    ...
    if(max(abs(theta_iter.row(count_num + 1) - theta_iter.row(count_num))) < 1e-6 || count_num == iter_num - 1){
      //if(count_num == iter_num - 1) Rcout << "Max iterion reached!" << endl; 
      break;
      }

    //Rcout<<count_num<<":"<<obj_vec(count_num)<<endl;
  }
  
  uvec supp_lambda = find(abs(vectorise(lambda)) > 0.001);
  mat theta_debiased = theta;
